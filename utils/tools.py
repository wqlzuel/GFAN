...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.lradj == 'type1':
        ...
        lr_adjust = {epoch: args.learning_rate if epoch < 10 | epoch % 10 != 0 else args.learning_rate*0.1}
    elif args.lradj == '3':
...
def visual(true, preds=None, name='./pic/test.pdf'):
    ...
    """
    plt.figure()
    plt.plot(true, label='GroundTruth', linewidth=2, color='#7E2F8E')
    if preds is not None:
        plt.plot(preds, label='Prediction', linewidth=2, color='#EDB120')
    plt.legend()
...
import os
def load_model(model, model_dir, model_name='ETTh1', horizon=12):
    if not model_dir:
        return
    file_name = os.path.join(model_dir, model_name+str(horizon)+'.bin')
